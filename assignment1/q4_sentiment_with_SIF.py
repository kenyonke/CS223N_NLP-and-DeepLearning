...
def outputPredictions(dataset, features, labels, clf, filename):
    ...
    with open(filename, "w") as f:
        ...
        for i in range(len(dataset)):
            print("%d\t%d\t%s" % (labels[i], pred[i], " ".join(dataset[i][0])), file=f)

def getSentenceFeaturesSIF(tokens, wordVectors, sentence, freq):
    """
    Obtain the sentence feature for sentiment analysis by smooth inverse frequency
    """
    ...
    for word in sentence:
        if word in tokens.keys():
            # vs += sif * word_vector
            length += 1
            sentVector += wordVectors[tokens[word]] * (a/(a+freq[word]))
