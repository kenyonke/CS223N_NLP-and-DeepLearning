...
def normalizeRows(x):
    ...
    unit length.
    """
    #Normalization method: x_n/sqrt(sum of all x_i ** 2)
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    gradPred = np.sum(outputVectors*prob.reshape(-1,1),axis=0) - outputVectors[target]
    
    #gradients for U(ont only o, but other context word would be contained)
    grad = np.tile(predicted, (outputVectors.shape[0],1)) * prob.reshape(-1,1)
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    V_c = [inputVectors[tokens[contextWord]] for contextWord in contextWords]
    V_c = np.sum(V_c,axis=0)
    cost,gradPred,gradOut = word2vecCostAndGradient(V_c, tokens[currentWord], outputVectors, dataset)
    for contextWord in contextWords:
